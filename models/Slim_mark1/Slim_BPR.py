...
class Slim_BPR_Recommender_Python(BPR_Sampling, RecommenderSystem, RecommenderSystem_SM):
    ...
    def fit(self, epochs=30,
        ...
        if self.sparse_weights:
            ...
            self.S = np.zeros((self.num_items, self.num_items)).astype('float32')

        self.initialize_fast_sampling(positive_threshold=self.positive_threshold)
    ...
    def fit_alreadyInitialized(self, epochs=30,
                               URM_test=None,
                               filterTopPop=False,
                               minRatingsPerUser=1,
                               batch_size=1000,
    ...
    def epochIteration(self):
        ...
        for numCurrentBatch in range(totalNumberOfBatch):
            ...
            if (time.time() - start_time_batch >= 30 or numCurrentBatch == totalNumberOfBatch - 1):
                ...
                start_time_batch = time.time()
        self.S[np.arange(0, self.num_items), np.arange(0, self.num_items)] = 0.0
