...
class AsySVD(RecommenderSystem):
    ...
    def fit(self, R):
        ...
        self.X, self.Y = MF.Cython.AsySVD_sgd(R, self.num_factors, self.lrate, self.reg, self.iters, self.init_mean,
                                    self.init_std,
                                    self.lrate_decay, self.rnd_seed)
        # precompute the user factors
        M = R.shape[0]
        self.U = np.vstack([MF.Cython.AsySVD_compute_user_factors(R[i], self.Y) for i in range(M)])
        self.parameters =  "num_factors={}, lrate={}, reg={}, iters={}, init_mean={}, " \
               "init_std={}, lrate_decay={}, rnd_seed={}".format(
